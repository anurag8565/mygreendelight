...
export default function BroadcastBar() {
  ...
  const typeStyles = {
    weather: {
      bg: "bg-gradient-to-r from-sky-600 via-blue-600 to-indigo-700 text-white",
    ...
    promo: {
      ...
      icon: <Sparkles size={16} className="shrink-0 text-yellow-300 animate-spin-slow" />,
      badge: "Special Offer",
    },
    info: {
    ...
    <div className={`w-full ${currentStyle.bg} py-2 px-3 sm:px-6 text-xs font-bold shadow-md relative z-40 transition-all duration-300`}>
      <div className="max-w-7xl mx-auto flex items-center justify-between gap-3">
        <div className="flex items-center gap-2.5 min-w-0 flex-1 justify-center sm:justify-start">
          ...
          <p className="truncate sm:whitespace-normal text-center sm:text-left font-extrabold text-[11px] sm:text-xs">
            {broadcast.message}
          </p>
          ...
          {broadcast.linkUrl && (
            <Link
              ...
              className="inline-flex items-center gap-1 bg-white text-gray-900 hover:bg-yellow-300 hover:text-black px-2.5 py-0.5 rounded-full text-[10px] sm:text-[11px] font-black transition-all shrink-0 ml-2 shadow-xs"
            >
        ...
        <button
          ...
          className="p-1 text-white/80 hover:text-white hover:bg-white/20 rounded-full transition shrink-0 cursor-pointer"
