  ...
  const Content = (
    <div className={`flex items-center gap-2.5 select-none group ${className}`}>
      ...
      <div className="flex flex-col leading-none font-[family-name:var(--font-brand-serif),serif] antialiased">
        ...
        {/* Refined Micro City Tagline (Farm Fresh removed as requested) */}
        {showTagline && !isCompact && (
          <div className="hidden sm:flex items-center gap-1 mt-0.5 font-sans">
            <span
              className={`text-[8.5px] sm:text-[9px] font-semibold tracking-widest uppercase ${
                isWhite
                  ? "text-emerald-100/80"
                  : isInvoice
                  ? "text-gray-400"
