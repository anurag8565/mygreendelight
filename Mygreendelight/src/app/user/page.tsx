...
export default function UserProfileHub() {
  ...
  return (
    <div className="min-h-screen bg-[#f8faf9] flex flex-col font-sans text-gray-900">
      ...
      <main className="flex-1 max-w-4xl w-full mx-auto px-3.5 sm:px-6 py-6 sm:py-10 pb-28 sm:pb-20 space-y-5">
        
        {/* Top Minimal Navigation Bar */}
        <div className="flex items-center justify-between">
          <Link
            href="/"
            className="inline-flex items-center gap-1.5 text-xs font-bold text-gray-600 hover:text-[#0f8646] transition bg-white px-3.5 py-2 rounded-xl border border-gray-200/90 shadow-2xs"
          >
            <ArrowLeft size={14} />
            <span>Back to Store</span>
          </Link>
          <span className="text-[11px] font-black text-[#0f8646] uppercase tracking-wider bg-emerald-50 px-3 py-1 rounded-full border border-emerald-200/80 flex items-center gap-1.5">
            <Sparkles size={13} /> Account Hub
          </span>
        </div>

        {/* 1. Ultra-Minimalist Profile Header Card */}
        <motion.div
        ...
        >
          {/* Subtle Ambient Emerald Disc in Background */}
          ...
          <div className="relative z-10 flex flex-col sm:flex-row items-center sm:items-start justify-between gap-5 text-center sm:text-left">
            <div className="flex flex-col sm:flex-row items-center gap-4 sm:gap-5">
              ...
              {/* Avatar Box */}
              <div className="relative w-18 h-18 sm:w-20 sm:h-20 rounded-2xl bg-gradient-to-tr from-emerald-600 to-emerald-400 text-white flex items-center justify-center font-black text-2xl sm:text-3xl shadow-md shrink-0 overflow-hidden border-2 border-white">
                {activeUser?.image ? (
                  <img
              ...
              <div className="min-w-0">
                <div className="flex items-center justify-center sm:justify-start gap-2 mb-1 flex-wrap">
                  ...
                  {activeUser?.role === "admin" ? (
                    <span className="bg-amber-100 text-amber-900 border border-amber-300 font-black text-[10px] uppercase px-2.5 py-0.5 rounded-full flex items-center gap-1">
                      <ShieldCheck size={11} className="text-amber-700" /> Admin
                    </span>
                  ) : activeUser?.role === "deliveryboy" ? (
                    <span className="bg-blue-100 text-blue-900 border border-blue-300 font-black text-[10px] uppercase px-2.5 py-0.5 rounded-full flex items-center gap-1">
                      <Truck size={11} className="text-blue-700" /> Rider
                    </span>
                  ) : (
                    <span className="bg-emerald-50 text-[#0f8646] border border-emerald-200 font-black text-[10px] uppercase px-2.5 py-0.5 rounded-full flex items-center gap-1">
                      <Sparkles size={11} /> Verified Bhopal Member
                    </span>
                  )}
                </div>
                ...
                <div className="flex items-center justify-center sm:justify-start gap-2.5 mt-2.5 text-[11px] text-gray-600 font-semibold flex-wrap">
                  <span className="flex items-center gap-1 text-emerald-700 bg-emerald-50/80 px-2 py-0.5 rounded-md border border-emerald-100">
                    <MapPin size={12} /> Bhopal, Madhya Pradesh
                  </span>
                  <span>•</span>
                  <span className="flex items-center gap-1 text-gray-500">
                    <Leaf size={12} className="text-[#0f8646]" /> 100% Farm Fresh
                  </span>
                </div>
              </div>
            </div>
            ...
            <button
              ...
              className="bg-gray-100/80 hover:bg-red-50 hover:text-red-700 hover:border-red-200 text-gray-700 font-black text-xs px-3.5 py-2 rounded-xl border border-gray-200/90 transition flex items-center gap-1.5 cursor-pointer shrink-0 shadow-2xs"
              title="Sign Out"
            >
              <LogOut size={13} />
              <span>Sign Out</span>
        ...
        {/* 3. Live Active Order Snip Banner (If user has pending/out of delivery order) */}
        {activeOrder && (
          <motion.div
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5">
                ...
                {activeUser?.role === "deliveryboy" && (
                  ...
                  >
                    ...
                    <ChevronRight size={16} className="text-[#0f8646] shrink-0" />
                  </Link>
                )}
              </div>
            </div>
        ...
        {/* 5. Minimal 24/7 Bhopal Concierge Support Card */}
        <div className="bg-white rounded-3xl p-5 sm:p-6 border border-gray-200/80 shadow-2xs flex flex-col sm:flex-row items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-2.5 w-full sm:w-auto">
            <a
              href="tel:9981418565"
              className="flex-1 sm:flex-initial bg-white hover:bg-gray-50 text-gray-800 border border-gray-300 px-4 py-2.5 rounded-xl font-black text-xs transition text-center shadow-2xs"
            >
              Call Support
            </a>
            <a
              ...
              className="flex-1 sm:flex-initial bg-[#25D366] hover:bg-[#20ba59] text-white px-4 py-2.5 rounded-xl font-black text-xs shadow-xs transition text-center flex items-center justify-center gap-1.5"
            >
